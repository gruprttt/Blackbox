...
(function () {
  ...
  function brainPage() {
    ...
    // progress in each), and open it in Tracks.
    ...
    function paint() {
      ...
      $("[data-level-sub]").textContent = d ? "Its topics and your progress in each. Open one to study it."
      ...
      if (d) {
        ...
        html += '<div class="lobe-card" style="--c:' + d.color + '"><p class="mono-label">Selected lobe</p><h3>' + esc(d.name) + "</h3><p>" + esc(d.blurb) + "</p>" +
          ...
          (d.soon ? '<p class="region-meta">Coming soon.</p>' : '<div class="lobe-actions"><a class="btn btn-primary sm" href="' + ROOT + "tracks/index.html#" + d.id + '">Open in Tracks</a></div>') + "</div>";
    ...
    document.addEventListener("keydown", function (e) {
      ...
      if (e.key === "Escape" && selLobe) select(null);
      if (e.key === "Enter" && selLobe && !lobeOf[selLobe].soon) location.href = ROOT + "tracks/index.html#" + selLobe;
    });
    BB.on(function (w) { if (w === "progress") paint(); });
    ...
    function drawGrowth() {
      ...
      var node = (selLobe && lobeNode[selLobe]) || TREE, g = chart.getContext("2d"), sp = span(), times = leafTimes(node), total = Math.max(1, tally(node).total);
