/* BLACKBOX skill map — tracks laid out as a prerequisite graph you can pan, zoom and expand. */
(function () {
  "use strict";
  var BB = window.BB, M = window.BB_MAP;
  if (!BB || !M) return;
  var $ = BB.$, $$ = BB.$$, esc = BB.esc, ROOT = BB.ROOT;
  var root = $("[data-mindmap]"), layer = $("[data-mm-layer]"), svg = $("[data-mm-edges]"), panel = $("[data-mm-panel]");
  if (!root) return;
  ...
  (BB.load("bb-map-open", []) || []).forEach(function (i) { open[i] = true; });
    ...
    M.tracks.forEach(function (t, i) {
      ...
      t.topics.forEach(function (tp) {
        tp.lessons.forEach(function (l) {
          var id = t.slug + "/" + tp.slug + "/" + l[0];
          total++;
          if (BB.isDone(id)) done++; else if (!firstOpen) firstOpen = id;
        });
      });
      st[i] = { done: done, total: total, open: firstOpen };
  ...
  function topicProgress(t, tp) {
    var d = 0;
    tp.lessons.forEach(function (l) { if (BB.isDone(t.slug + "/" + tp.slug + "/" + l[0])) d++; });
    return d;
  }
  ...
  function render() {
    ...
    M.tracks.forEach(function (t, i) {
      ...
      html += '<div class="mm-node ' + cls + (open[i] ? " is-open" : "") + '" data-node="' + i + '" style="--c:' + t.color + ";transform:translate(" + pos[i].x + "px," + pos[i].y + 'px)" tabindex="0" role="button" aria-label="Track ' + t.num + ": " + esc(t.title) + '">' +
        '<div class="mm-kicker"><span>Track ' + t.num + '</span><span class="mm-state">' + (cls === "s-done" ? "completed" : cls === "s-cur" ? "you are here" : cls === "s-start" ? "started" : "") + "</span></div>" +
        "<h3>" + esc(t.title) + "</h3>" +
        '<p class="mm-q">“' + esc(t.q) + "”</p>" +
        '<div class="mm-foot"><span>' + t.topics.length + ' topics</span><span class="mm-bar"><i style="width:' + (pct * 100).toFixed(1) + '%"></i></span><span>' + Math.round(pct * 100) + "%</span></div>" +
  ...
  root.addEventListener("click", function (e) {
    ...
    if (tog) {
      ...
      open[i] = !open[i];
      BB.save("bb-map-open", Object.keys(open).filter(function (k) { return open[k]; }).map(Number));
      render(); apply();
      return;
    }
  ...
  function showTrack(i) {
    ...
    panel.innerHTML = '<div class="mp-head"><span class="mono-label">Track ' + t.num + " · " + esc(t.domain) + '</span><button class="icon-btn" data-close aria-label="Close">✕</button></div>' +
      "<h2>" + esc(t.title) + '</h2><p class="mp-q">“' + esc(t.q) + '”</p>' +
      '<div class="mp-prog"><div class="progress-bar"><span style="--p:' + (pct / 100) + '"></span></div><span class="mono-label">' + s.done + "/" + s.total + " · " + pct + "%</span></div>" +
      '<div class="mp-actions"><a class="btn btn-primary sm" href="' + ROOT + (s.open || t.slug + "/" + t.topics[0].slug + "/" + t.topics[0].lessons[0][0]) + '/index.html">' +
      (s.done >= s.total ? "Review" : s.done ? "Continue" : "Start") + ' track</a><a class="btn btn-ghost sm" href="' + ROOT + t.slug + '/index.html">Track page</a></div>' +
      '<p class="mono-label mp-sub">Topics</p><ul class="mp-list">' + t.topics.map(function (tp, j) {
        var d = topicProgress(t, tp), tot = tp.lessons.length;
  ...
  function showTopic(i, j) {
    ...
    panel.hidden = false;
    panel.style.setProperty("--c", t.color);
    panel.innerHTML = '<div class="mp-head"><button class="link-btn" data-back="' + i + '">← Track ' + t.num + '</button><button class="icon-btn" data-close aria-label="Close">✕</button></div>' +
      '<span class="mono-label">Topic ' + (j + 1) + " of " + t.topics.length + "</span><h2>" + esc(tp.title) + "</h2>" +
      '<div class="mp-prog"><div class="progress-bar"><span style="--p:' + (d / tot) + '"></span></div><span class="mono-label">' + d + "/" + tot + " lessons</span></div>" +
      '<ol class="mp-lessons">' + tp.lessons.map(function (l, k) {
        var id = t.slug + "/" + tp.slug + "/" + l[0], done = BB.isDone(id);
        return '<li class="' + (done ? "done" : "") + '"><a href="' + ROOT + id + '/index.html"><span class="lnum"><b>' + (k + 1) + '</b><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="m5 12.5 4.5 4.5L19 7.5"/></svg></span><span>' +
          esc(l[1]) + "</span><em>" + l[2] + "m</em></a></li>";
      }).join("") + "</ol>" +
      '<a class="btn btn-ghost sm" href="' + ROOT + t.slug + "/" + tp.slug + '/index.html">Open topic page</a>';
    wirePanel();
  }
  function wirePanel() {
  ...
  var st0 = trackState();
  fit();
  if (root.clientWidth < 700) centerOn(Math.max(0, st0.current), 0.78);            // phones: start readable, on your track
  else if (st0.current > 0) centerOn(st0.current, Math.max(0.7, view.k));
  if ("ResizeObserver" in window) new ResizeObserver(function () { if (!root.clientWidth) return; }).observe(root);
  BB.on(function (w) { if (w === "progress") { render(); apply(); } });
  window.BB_MINDMAP = { resize: function () { if (root.clientWidth) fit(); } };
})();
